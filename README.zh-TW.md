...
1. 建立範本：`SHEET_ID=xxx node scripts/create-template.mjs`
2. 刪除範例資料，輸入你自己的交易紀錄
...
4. `cp .env.example .env.local` 並填入你的設定值
5. `npm install && npm run dev`
6. 透過 Cloudflare Access 部署（參見 [docs/deploy.md](docs/deploy.md)）

## 技術架構

- Google Sheets（GOOGLEFINANCE、SUMIFS、UNIQUE、XLOOKUP）
- Next.js 14 + Tailwind CSS + shadcn/ui + Recharts
- ISR（每 10 分鐘重新驗證，配合 GOOGLEFINANCE 延遲）
- PWA 離線支援
